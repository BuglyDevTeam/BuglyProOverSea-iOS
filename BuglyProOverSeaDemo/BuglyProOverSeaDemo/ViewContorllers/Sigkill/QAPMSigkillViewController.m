...
- (void)setupViews {
    ...
    [_makeVMMallocBtn addTarget:self action:@selector(vmMallocStackMemoryOOM)
    ...
    self.memLabelTimer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:[YYWeakProxy proxyWithTarget:self]
                                                        selector:@selector(updateMemLabel)
                                                        userInfo:nil
                                                         repeats:YES];
...
- (void)mallocStackMemoryOOM {
    self.timer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:[YYWeakProxy proxyWithTarget:self]
...
- (void)vmMallocStackMemoryOOM {
    self.timer = [NSTimer scheduledTimerWithTimeInterval:1 target:[YYWeakProxy proxyWithTarget:self]
...
- (void)updateMemLabel {
    ...
    //    NSLog(@"当前footprint memory: %.2f MB", [self memoryUsage] / 1024.0 / 1024.0 );
    //    NSLog(@"当前vm memory: %.2f MB", [self vm_memoryUsage] / 1024.0 / 1024.0 );
    //    NSLog(@"当前rs memory: %.2f MB", [self rs_memoryUsage] / 1024.0 / 1024.0 );
    //    NSLog(@"当前debug navigator memory: %.2f MB", [self usedSizeOfMemory] );
}
